...
class ParkingTest extends TestCase
{
    ...
    public function testUserCanStartParking()
    {
        ...
        $response = $this->actingAs($user)->postJson('/api/v1/parking/start', [
    ...
    public function testUserCanGetOngoingParkingWithCorrectPrice()
    {
        ...
        $this->actingAs($user)->postJson('/api/v1/parking/start', [
            'vehicle_id' => $vehicle->id,
            'zone_id'    => $zone->id,
        ]);
        ...
        $response = $this->actingAs($user)->getJson('/api/v1/parking/' . $parking->id);
    ...
    public function testUserCanStopParking()
    {
        ...
        $parking = Parking::first();
        $response = $this->actingAs($user)->putJson('/api/v1/parking/' . $parking->id);
